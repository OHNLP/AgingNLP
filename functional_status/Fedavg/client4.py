...
def load_data():
	...
	epochs_num = 5
	df = pd.read_csv('/infodev1/phi-projects/dhs/workspace-sf/MCI/BERT_data/data/NLP_input/fl/training_ut.txt', delimiter= '\t', header=None, names=['docid', 'sentence', 'label'])
...
# 	model.to(device)
	...
	class MCIClient4(fl.client.NumPyClient):
		...
		def evaluate(self, parameters, config):
			self.set_parameters(parameters)
			loss, accuracy = test(model, testloader, epochs=5)
			return float(loss), len(testloader), {"accuracy": float(accuracy)}

    # Start client
	fl.client.start_numpy_client(server_address="127.0.0.1:8080", client=MCIClient4())
