...
# 	df = pd.read_csv('/infodev1/phi-projects/dhs/workspace-sf/MCI/BERT_data/data/NLP_input/fl/training_mchs_dedu2.txt', delimiter= '\t', header=None, names=['docid', 'sentence', 'label'])	
	df = df.dropna()
...
# 			'Training Time': training_time,
	...
	output_dir = './output_models/site_mchs/bert_output/'	
...
# 		"emilyalsentzer/Bio_ClinicalBERT",
    ...
    # Flower client
	class MCIClient3(fl.client.NumPyClient):
		...
		def evaluate(self, parameters, config):
			self.set_parameters(parameters)
			loss, accuracy = test(model, testloader, epochs=3)
			return float(loss), len(testloader), {"accuracy": float(accuracy)}

    # Start client
	fl.client.start_numpy_client(server_address="127.0.0.1:8080", client=MCIClient3())
