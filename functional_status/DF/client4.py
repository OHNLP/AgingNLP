...
def load_data():
	...
	epochs_num = 5
	df = pd.read_csv('/data/sfu3/workspace/mci/data/training_ut.csv', delimiter= '\t', header=None, names=['docid', 'sentence', 'label'])
...
# 			'Training Time': training_time,
	...
	output_dir = './output_models/site_ut/bert_output/'	
...
# 	model.to(device)
    ...
    # Flower client
	class MCIClient4(fl.client.NumPyClient):
		...
		def evaluate(self, parameters, config):
			self.set_parameters(parameters)
			loss, accuracy = test(model, testloader, epochs=1)
			return float(loss), len(testloader), {"accuracy": float(accuracy)}

    # Start client
	client = MCIClient4()
	client = DPFedAvgNumPyClient(client)
	fl.client.start_numpy_client(server_address="127.0.0.1:8080", client=client, grpc_max_message_length=995896761)
