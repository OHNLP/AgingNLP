...
def create_combined_xml(raw_notes, nlp_output):
	...
	for line in nlp_output.strip().splitlines():
		...
		"spans": "{}~{}".format(parts[3].split('=')[1].strip('"'), parts[4].split('=')[1].strip('"')),
		"certainty": parts[5].split('=')[1].strip('"').lower(),
		"status": parts[6].split('=')[1].strip('"').lower(),
		"experiencer": parts[7].split('=')[1].strip('"').lower(),
		"id": parts[2].split('=')[1].strip('"')[:2].upper() + str(nlp_output.strip().splitlines().index(line)),
		# "CAM_criteria": "",
		"exclusion": "",
		"comment": ""
		}
		etree.SubElement(tags_element, parts[8].split('=')[1].strip('"').capitalize(), tag_attributes)
	rough_string = etree.tostring(root, pretty_print=True, encoding='UTF-8').decode('utf-8')
	return rough_string
